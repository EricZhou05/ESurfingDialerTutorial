...
借助第三方 [广东电信天翼校园（ZSM验证）登入认证客户端](https://github.com/Rsplwe/ESurfingDialer)，实现自动认证联网的效果。配合软路由 Docker 部署可实现多设备共享。脚本原作者的说明比较简略难懂较为精简，下面的教程是自己一步步摸索过来的，提供给后面的学弟学妹，希望能让大家少走一些弯路。

## 声明！<(｀^´)>
允许随意转载引用，但是请同学们符合本文使用的AGPL-3.0规范喔：

- 引用与版权声明：必须在你的作品中注明原作者并附带AGPL-3.0协议内容。

- 内容修改与授权：即便修改了部分内容，只要原始文章的核心内容或结构没有显著改变，并且基于本文创作，那么仍然需要遵循AGPL-3.0协议，并且应该明确注明本文作为来源。

- 协议传递性：你的作品也应该继续在AGPL-3.0协议下发布。
## 运行环境
